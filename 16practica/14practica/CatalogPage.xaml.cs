...
public partial class CatalogPage : Window
{
    ...
    private void LoadCatalog()
    {
        ...
        catch (Exception ex)
        {
            ...
            for (var i = 1; i <= 8; i++)
            {
                ...
                {
                    Id = i,
                    Title = $"Демо-карточка {i}",
                    ShortDescription = "Локальная заглушка при недоступной БД",
                    StatusName = "Демо",
                    Rating = i % 10 + 1,
    ...
    private static IEnumerable<CatalogItemViewModel> ReadFromDatabase()
    {
        ...
        while (reader.Read())
        {
            ...
            var mediaType = reader.IsDBNull(2) ? "Медиа" : reader.GetString(2);
            var year = reader.IsDBNull(3) ? 0 : reader.GetInt32(3);
            var genre = reader.IsDBNull(4) ? "Жанр не указан" : reader.GetString(4);
            var status = reader.IsDBNull(5) ? "Без статуса" : reader.GetString(5);

            byte[]? imageBytes = null;
            if (!reader.IsDBNull(6))
            {
                imageBytes = (byte[])reader[6];
            }
            ...
            yield return new CatalogItemViewModel
            {
                Id = id,
                Title = title,
                ShortDescription = $"{mediaType}, {year}. {genre}",
                StatusName = status,
                Rating = id % 10 + 1,
                Difficulty = GetDifficultyLabel(id),
                Poster = CreateImage(imageBytes)
            };
        }
    }
